...
def _validate(model, teacher_model, val_dataloader, device):
    ...
    for step_idx, batch_data in tqdm(enumerate(val_dataloader), f"[Eval] ", total=len(val_dataloader)):
        with torch.no_grad():
            ...
            embed_gt = teacher_model(text_gt.to(device))[0]
            
            loss_en = loss_fn(embed_en, embed_gt)
            loss_ko = loss_fn(embed_ko, embed_gt)
...
text_encoder_student = CLIPTextModel.from_pretrained("/home/sangyeob/dev/d2d/5-K_stable_diffusion/text_encoder")
...
num_total_steps = step_per_epoch * epochs
model_path = f'/home/sangyeob/dev/d2d/5-K_stable_diffusion/text_encoder/robert/{total_train_dataset_len + total_valid_dataset_len}_{learning_rate}_{batch_size}'
